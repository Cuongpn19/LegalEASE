                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="avatar-sm mr-3 bg-soft-primary"
                                style="width: 40px; height: 40px; border-radius: 50%; background: #e0e7ff; display: flex; align-items: center; justify-content: center; color: #4e73df; font-weight: bold;">
                                AN
                            </div>
                            <div>
                                <span class="font-weight-bold d-block text-dark">{{ $d->client?->name ?? 'Khách lạ' }}</span>
                            </div>
                        </div>
                    </td>
                    <td>{{ $d->appointment_date }}</td>
                    <td>
                        <span class="badge badge-pill badge-soft-secondary px-3 py-2">{{ $d->status }}</span>
                    </td>
                    <td class="text-muted">{{ $d->notes }}</td>
                    <td class="text-right">
                        <form action="{{ Route('lawyer.done') }}" method="post"
                            onsubmit="return confirm('You confirm that you have advised the client [{{ $d->client->name }}]? ')">
                            @csrf
